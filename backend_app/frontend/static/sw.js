...
async function handlePush(event) {
  ...
  const chatStr = chatId !== null && chatId !== undefined ? String(chatId) : null;

  const senderUsername = String(data.sender_username || "").trim();
  const senderDisplay = String(
    data.sender_display || (senderUsername ? "@" + (senderUsername.startsWith("@") ? senderUsername.slice(1) : senderUsername) : "Новое сообщение")
  ).trim();

  const title = String(data.title || senderDisplay || "Новое сообщение").trim();

  const body = String(data.body || data.text || "").trim();

  const icon = String(data.avatar_icon_url || data.icon || "/static/icon-192.png").trim();
  const badge = String(data.badge || icon).trim();
  ...
  try {
    ...
    const visibleClients = clientsArr.filter((c) => c && c.visibilityState === "visible");
  ...
  const options = {
    ...
    tag: chatStr ? "chat:" + chatStr : undefined,
    renotify: false,
    data: {
      chatId: chatId,
      chat_id: chatId,
...
self.addEventListener("notificationclick", (event) => {
  ...
  const chatId = event.notification?.data?.chatId ?? event.notification?.data?.chat_id ?? null;
