...
let _webPushInited = false;
let _webPushPublicKey = null;
let _webPushInitPromise = null;
...
async function _fetchVapidPublicKey() {
  if (_webPushPublicKey) return _webPushPublicKey;

  const r = await fetch(API + "/push/vapid_public_key");
  if (!r.ok) throw new Error("Cannot load VAPID public key");
  const j = await r.json();
  if (!j || !j.publicKey) throw new Error("Bad VAPID public key response");
  _webPushPublicKey = String(j.publicKey);
  return _webPushPublicKey;
}

async function _ensureServiceWorker() {
  if (!("serviceWorker" in navigator)) throw new Error("Service Worker not supported");
  // SW лежит в корне (/sw.js) — scope будет "/"
...
async function _getCurrentSubscription(reg) {
  try {
...
function _subToPlainObject(sub) {
  const j = sub.toJSON ? sub.toJSON() : null;
  if (j && j.endpoint && j.keys && j.keys.p256dh && j.keys.auth) return j;

  // fallback
  const keys = sub.getKey ? sub.getKey.bind(sub) : null;
...
async function _sendSubscriptionToServer(subPlain) {
  if (!token) return;
  await fetch(API + "/push/subscribe", {
    method: "POST",
    headers: authHeadersJson(),
    body: JSON.stringify({
      endpoint: subPlain.endpoint,
      keys: {
        p256dh: subPlain.keys.p256dh,
        auth: subPlain.keys.auth,
      },
...
async function ensureWebPushSubscribed() {
  // Только когда есть login
  if (!token || !me) return;

  // Только HTTPS или localhost (на Railway HTTPS)
  const isSecure =
  ...
  if (_webPushInitPromise) return _webPushInitPromise;

  _webPushInitPromise = (async () => {
    if (_webPushInited) return;

    const okPerm = await _ensureNotificationPermission();
    if (!okPerm) return;

    const reg = await _ensureServiceWorker();
    const pub = await _fetchVapidPublicKey();

    let sub = await _getCurrentSubscription(reg);
    if (!sub) {
    ...
    const plain = _subToPlainObject(sub);
    await _sendSubscriptionToServer(plain);

    _webPushInited = true;
  })()
    .catch(() => {
      // не ломаем приложение если push не взлетел
    })
    .finally(() => {
      // разрешаем повторную попытку позже
      _webPushInitPromise = null;
    });

  return _webPushInitPromise;
}

async function tryUnsubscribeWebPush() {
  try {
    ...
    // сообщим серверу
    if (token) {
      const url = new URL(API + "/push/unsubscribe", location.origin);
      url.searchParams.set("endpoint", endpoint);
      await fetch(url.toString(), { method: "POST", headers: authHeadersJson() });
    }
  } catch (_) {}
}
...
function _keyUnread() {
  return `${LS_UNREAD_BASE}_${_persistScope()}`;
}
function _keyDrafts() {
  return `${LS_DRAFTS_BASE}_${_persistScope()}`;
}
...
function resetDialogsStateAndUI() {
  stopDialogsLive();
  if (dialogsReloadTimer) {
    clearTimeout(dialogsReloadTimer);
...
async function assistantRequestSuggestion(reason = "idle") {
  ...
  if (assistantBusy) assistantCancelInFlight();
...
async function register() {
  ...
  if (j && j.access_token) {
    ...
    // ✅ Web Push subscribe (после user gesture)
...
function logout() {
  // ✅ попытка отписки (не критично)
  tryUnsubscribeWebPush();
...
function _paintDialogPreviewBits(rowEl, lastPreview) {
  ...
  if (iconEl) {
    const shouldShowIcon = !!(
      lastPreview?.kind &&
      String(lastPreview?.text || "").trim() === (lastPreview.kind.label || "")
    );
...
function renderDialogRowSkeleton({ chatId, other, online, lastPreview, hasUnread }) {
  ...
  const onlineDotEl = mk("span", { class: `dlgOnlineDot ${online ? "online" : ""}` });
  avatarWrap.appendChild(onlineDotEl);
  ...
  const txt = mk("span", {
    class: isMe ? "dlgText sent" : "dlgText recv",
    text: lastPreview?.text || "—",
  });
...
function updateDialogPreviewFromMessage(chatId, msg, { moveToTop = true } = {}) {
  ...
  if (nextMsgId && prevMsgId && nextMsgId < prevMsgId) return;
...
function renderAttachmentsNode(atts) {
  ...
  for (const a of atts) {
    ...
    const link = mk("a", { href: url, target: "_blank", rel: "noopener" }, [`📎 ${a.name || "file"}`]);
...
async function openChat(chatId, otherId, title) {
  ...
  const other =
    otherByChatId.get(cid) || { id: otherId, username: title, avatar_url: null, avatar_file_id: null };
...
function bindUI() {
  ...
  sfRemove &&
    sfRemove.addEventListener("click", () => {
      if (currentUploadXhr) {
        try {
