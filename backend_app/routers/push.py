...
def _require_vapid():
    # после нормализации в config.py тут уже будет норм PEM
    ...
    # доп. проверка: чтобы быстро видеть проблемы в логах
    if not settings.VAPID_PRIVATE_KEY_PEM.lstrip().startswith("-----BEGIN"):
        raise HTTPException(500, "VAPID_PRIVATE_KEY_PEM is not a PEM key (BEGIN header not found)")
    ...
    for s in subs:
        try:
            webpush(
                ...
                data=payload,
                vapid_private_key=settings.VAPID_PRIVATE_KEY_PEM,  # уже нормализован
